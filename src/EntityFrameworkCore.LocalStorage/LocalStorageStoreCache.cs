...
namespace EntityFrameworkCore.LocalStorage.NewFolder
{
    public class LocalStorageStoreCache : IFileContextStoreCache
    {
        ...
        public virtual IFileContextStore GetStore(IFileContextScopedOptions options)
        {
            ...
            {
                return _namedStores.GetOrAdd(options, _ => new LocalStorageContextStore(new LocalStorageTableFactory(_loggingOptions, options, _localStorage), _useNameMatching));
            }
            else
            {
                return _namedStores.GetOrAdd(options, _ => new LocalStorageContextStore(new FileContextTableFactory(_loggingOptions, options), _useNameMatching));
            }
        }
    }
}
